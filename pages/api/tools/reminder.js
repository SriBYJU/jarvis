...
export default async function handler(req, res) {
  if (req.method === "POST") {
    const { action, time, task, seconds, type, id } = req.body;
    ...
    if (action === "create") {
      ...
      const entry = await addReminder({ task, fireAt, type: type || "immediate" });
      return res.status(200).json({ type: "reminder", data: { ...entry, fireAt, timeStr: time || "" } });
    }
