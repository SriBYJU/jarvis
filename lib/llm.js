...
// Global provider-level rate limit (e.g. OpenRouter daily limit)
let openRouterGlobalBlock = 0;
const GLOBAL_BLOCK_MS = 300000; // 5 min block when daily limit hit

function isProviderBlocked() {
  if (!openRouterGlobalBlock) return false;
  if (Date.now() > openRouterGlobalBlock) { openRouterGlobalBlock = 0; return false; }
  return true;
}
...
async function callOpenRouter(messages, systemPrompt, model) {
  ...
  if (isProviderBlocked()) return null;
  ...
  try {
    ...
    if (res.status === 429) {
      ...
      // Check for global daily limit
      try {
        const errData = await res.clone().json();
        if (errData.error?.message?.includes("free-models-per-day") || errData.error?.message?.includes("Add 10 credits")) {
          openRouterGlobalBlock = Date.now() + GLOBAL_BLOCK_MS;
        }
      } catch {}
      return null;
    }
    ...
    if (data.error) {
      if (data.error.code === 429 || (data.error.message || "").includes("rate")) {
        markRateLimited(model);
        if ((data.error.message || "").includes("free-models-per-day")) openRouterGlobalBlock = Date.now() + GLOBAL_BLOCK_MS;
      }
      return null;
    }
...
// FreeAI — 50+ models, OpenAI-compatible, no API key needed
async function callFreeAI(messages, systemPrompt) {
  const models = ["gpt-4o-mini", "gpt-4o", "claude-3-haiku", "llama-3.1-8b", "mistral-small"];
  const formatted = [{ role: "system", content: systemPrompt }, ...messages];

  for (const model of models) {
    try {
      const res = await fetch("https://api.free-ai.org/v1/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model, messages: formatted, max_tokens: 1024, temperature: 0.7 }),
      ...
      if (text) {
        const cleaned = sanitizeReply(text);
        if (cleaned) return { text: cleaned, model: `free-ai/${model}` };
      }
    } catch {
      continue;
    }
...
export async function chatCompletion(messages, systemPrompt, mode = "fast", specificModel = null) {
  ...
  // FreeAI — 50+ models, last resort
  try {
    const freeResult = await callFreeAI(messages, systemPrompt);
    if (freeResult) {
      setCache(cacheKey, freeResult.text);
      return { reply: freeResult.text, model: freeResult.model };
    }
  } catch {}
