const INTENT_PATTERNS = {
  weather: {
    regex: /(?:weather|temperature|forecast|rain|snow|humid)\s+(?:in|at|for|of)\s+(.+)/i,
    extract: (msg) => {
      const m = msg.match(/(?:weather|temperature|forecast|rain|snow|humid)\s+(?:in|at|for|of)\s+(.+)/i);
  ...
  youtube: {
    regex: /(?:youtube|play\s+(?:a\s+)?video|watch|video\s+of|play\s+me)\s+(.+)/i,
    extract: (msg) => {
      const m = msg.match(/(?:youtube|play\s+(?:a\s+)?video\s+(?:of\s+|about\s+)?|watch\s+|video\s+of\s+|play\s+me\s+)(.+)/i);
      return m ? m[1].trim() : null;
    },
  },
  timer: {
  ...
  worldclock: {
    regex: /(?:world\s+clocks?|time\s+in|what\s+time|current\s+time)/i,
    extract: (msg) => {
      const m = msg.match(/(?:time|what\s+time\s+is\s+it)\s+in\s+(.+)/i);
      return m ? m[1].trim() : "world";
    },
  },
  joke: {
    regex: /(?:joke|tell\s+me\s+a\s+joke|make\s+me\s+laugh|something\s+funny)/i,
    extract: () => true,
  },
  memory_save: {
  ...
  stock: {
    regex: /(?:stock|share\s+price|ticker|stock\s+price|market\s+price)\s+(?:of\s+|for\s+)?(.+)/i,
    extract: (msg) => {
      const m = msg.match(/(?:stock|share\s+price|ticker|stock\s+price|market\s+price)\s+(?:of\s+|for\s+)?(.+)/i);
  ...
  news: {
    regex: /(?:news|headlines|latest\s+news|top\s+stories|breaking\s+news)\s*(?:about\s+|on\s+|for\s+|around\s+|in\s+|near\s+)?(.+)?/i,
    extract: (msg) => {
      const m = msg.match(/(?:news|headlines|latest\s+news|top\s+stories|breaking\s+news)\s*(?:about\s+|on\s+|for\s+|around\s+|in\s+|near\s+)?(.+)?/i);
      return m && m[1] ? m[1].trim() : "top";
    },
  },
  image: {
...
const NEWS_SIGNALS = ["news", "headlines", "stories", "breaking", "latest news", "local news", "top stories"];
const WEATHER_SIGNALS = ["weather", "temperature", "forecast", "humid", "rain chance", "snow"];
