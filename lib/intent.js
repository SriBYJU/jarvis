const INTENT_PATTERNS = {
  ...
  open_app: {
    regex: /(?:open|launch|start|run)\s+(?:up\s+)?(?:the\s+)?(.+?)(?:\s+(?:app|application|program))?$/i,
    extract: (msg) => {
      const m = msg.match(/(?:open|launch|start|run)\s+(?:up\s+)?(?:the\s+)?(.+?)(?:\s+(?:app|application|program))?$/i);
      if (!m) return null;
  ...
  save_file: {
    regex: /(?:save|write|create)\s+(?:this|a\s+file|the\s+file|to)\s+(?:to\s+|as\s+|at\s+)?(.+)/i,
    extract: (msg) => {
      const m = msg.match(/(?:save|write|create)\s+(?:this|a\s+file|the\s+file|to)\s+(?:to\s+|as\s+|at\s+)?(.+)/i);
      return m ? m[1].trim() : null;
    },
  },
  list_files: {
    regex: /(?:list|show|what(?:'s|\s+is)\s+in|browse)\s+(?:my\s+)?(?:files?|folder|directory|desktop|documents|downloads)\s*(?:in\s+)?(.+)?/i,
    extract: (msg) => {
      const m = msg.match(/(?:list|show|what(?:'s|\s+is)\s+in|browse)\s+(?:my\s+)?(?:files?|folder|directory|desktop|documents|downloads)\s*(?:in\s+)?(.+)?/i);
      if (m?.[1]) return m[1].trim();
      if (/desktop/i.test(msg)) return "~/Desktop";
      if (/documents/i.test(msg)) return "~/Documents";
      if (/downloads/i.test(msg)) return "~/Downloads";
      return "~";
    },
  },
  run_local: {
    regex: /(?:run|execute)\s+(?:this\s+)?(?:locally|on\s+my\s+(?:machine|computer|pc))\s*:?\s*(.+)?/i,
    extract: (msg) => {
      const m = msg.match(/(?:run|execute)\s+(?:this\s+)?(?:locally|on\s+my\s+(?:machine|computer|pc))\s*:?\s*(.+)?/i);
      return m?.[1]?.trim() || null;
    },
  },
  make_spreadsheet: {
    regex: /(?:make|create|build|generate)\s+(?:me\s+)?(?:a\s+)?(?:spreadsheet|csv|excel|table|sheet)\s*(?:with|of|about|for|called|named)?\s*(.*)?/i,
    extract: (msg) => {
      const m = msg.match(/(?:make|create|build|generate)\s+(?:me\s+)?(?:a\s+)?(?:spreadsheet|csv|excel|table|sheet)\s*(?:with|of|about|for|called|named)?\s*(.*)?/i);
      return m?.[1]?.trim() || null;
    },
  },
  make_document: {
    regex: /(?:make|create|write|draft|generate)\s+(?:me\s+)?(?:a\s+)?(?:document|doc|report|note|letter|essay|summary|brief)\s*(?:about|on|for|called|named|regarding|titled)?\s*(.*)?/i,
    extract: (msg) => {
      const m = msg.match(/(?:make|create|write|draft|generate)\s+(?:me\s+)?(?:a\s+)?(?:document|doc|report|note|letter|essay|summary|brief)\s*(?:about|on|for|called|named|regarding|titled)?\s*(.*)?/i);
      return m?.[1]?.trim() || null;
    },
  },
  organize_files: {
    regex: /(?:organize|clean\s+up|sort|tidy)\s+(?:my\s+)?(?:files?|folder|downloads?|desktop|directory)\s*(?:in\s+)?(.+)?/i,
    extract: (msg) => {
      const m = msg.match(/(?:organize|clean\s+up|sort|tidy)\s+(?:my\s+)?(?:files?|folder|downloads?|desktop|directory)\s*(?:in\s+)?(.+)?/i);
      if (/downloads?/i.test(msg)) return "~/Downloads";
      if (/desktop/i.test(msg)) return "~/Desktop";
      return m?.[1]?.trim() || "~/Downloads";
    },
  },
  search_files: {
    regex: /(?:find|search|look\s+for|locate)\s+(?:the\s+)?(?:file|files?)\s+(?:named|called)?\s*(.+)/i,
    extract: (msg) => {
      const m = msg.match(/(?:find|search|look\s+for|locate)\s+(?:the\s+)?(?:file|files?)\s+(?:named|called)?\s*(.+)/i);
      return m?.[1]?.trim() || null;
    },
  },
  clipboard: {
    regex: /(?:copy\s+(?:this|that|it)|paste|clipboard|what(?:'s|\s+is)\s+(?:in\s+)?(?:my\s+)?clipboard|copy\s+to\s+clipboard)\s*(.*)?/i,
    extract: (msg) => {
      if (/paste|what(?:'s|\s+is)\s+(?:in\s+)?(?:my\s+)?clipboard/i.test(msg)) return { action: "paste" };
      const m = msg.match(/copy\s+(?:this|that|it|to\s+clipboard)?\s*[:\s]*(.+)?/i);
      return { action: "copy", text: m?.[1]?.trim() || "" };
    },
  },
};
...
export function detectIntent(message) {
  ...
  if (INTENT_PATTERNS.run_local.regex.test(msg)) {
    return { intent: "run_local", data: INTENT_PATTERNS.run_local.extract(msg) };
  }
  if (INTENT_PATTERNS.list_files.regex.test(msg)) {
    return { intent: "list_files", data: INTENT_PATTERNS.list_files.extract(msg) };
  }
  if (INTENT_PATTERNS.save_file.regex.test(msg)) {
    return { intent: "save_file", data: INTENT_PATTERNS.save_file.extract(msg) };
  }
  if (INTENT_PATTERNS.make_spreadsheet.regex.test(msg)) {
    return { intent: "make_spreadsheet", data: INTENT_PATTERNS.make_spreadsheet.extract(msg) };
  }
  if (INTENT_PATTERNS.make_document.regex.test(msg)) {
    return { intent: "make_document", data: INTENT_PATTERNS.make_document.extract(msg) };
  }
  if (INTENT_PATTERNS.organize_files.regex.test(msg)) {
    return { intent: "organize_files", data: INTENT_PATTERNS.organize_files.extract(msg) };
  }
  if (INTENT_PATTERNS.search_files.regex.test(msg)) {
    return { intent: "search_files", data: INTENT_PATTERNS.search_files.extract(msg) };
  }
  if (INTENT_PATTERNS.clipboard.regex.test(msg)) {
    return { intent: "clipboard", data: INTENT_PATTERNS.clipboard.extract(msg) };
  }
  ...
  // Open app: low priority since "open" is common
  if (INTENT_PATTERNS.open_app.regex.test(msg) && /\b(open|launch|start)\b/i.test(msg)) {
    const data = INTENT_PATTERNS.open_app.extract(msg);
    if (data && data.target && data.target.length < 40) return { intent: "open_app", data };
  }
